...
List<VoucherView> extractVouchers(Map<String, dynamic> order) {
  ...
  for (final rawItem in _asList(order['items'])) {
    ...
    final isScratched = item['is_scratched'] == true;
    final isGift = item['is_gift'] == true;
...
double orderTotalAmount(Map<String, dynamic> order) {
  ...
  if (pricing['final_payable'] != null) return _toDouble(pricing['final_payable']);
...
String orderBrandName(Map<String, dynamic> order) {
  ...
  final suffix =
      orderNumber.length > 8 ? orderNumber.substring(orderNumber.length - 8) : orderNumber;
  return 'Order #$suffix';
}

String? orderRedeemSteps(Map<String, dynamic> order) {
  final meta = firstItemMeta(order);
  final v = meta['redeem_steps'] ?? meta['RedeemSteps'] ?? meta['how_to_redeem'];
  final s = v?.toString();
...
String? orderItemImageUrl(Map<String, dynamic> meta) {
  ...
  final brandId = meta['BrandId']?.toString() ?? meta['brandId']?.toString() ?? meta['brand_id']?.toString();
...
String orderStatus(Map<String, dynamic> order) => (order['status']?.toString() ?? '').toUpperCase();
