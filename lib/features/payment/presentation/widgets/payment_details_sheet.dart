...
class _PaymentDetailsSheetState extends State<PaymentDetailsSheet>
  ...
  Widget build(BuildContext context) {
    ...
    final denominations = _parseDenominations(
      brand.denominationList ?? brand.denomination,
    );

    // Auto-select when minPrice == maxPrice (single fixed price, e.g. "299-299")
    if (minPrice > 0 &&
        minPrice == maxPrice &&
        (_selectedAmount == null || _selectedAmount!.isEmpty)) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted)
          setState(() => _selectedAmount = minPrice.toInt().toString());
      });
    }
    ...
    if (isFixed &&
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted)
          setState(
            () => _selectedAmount = denominations.first.toInt().toString(),
          );
      });
    }
    // Auto-select minPrice for variable (range) brands
    if (!isFixed && minPrice > 0 && minPrice != maxPrice &&
        (_selectedAmount == null || _selectedAmount!.isEmpty)) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted)
          setState(() => _selectedAmount = minPrice.toInt().toString());
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    ...
                    _buildBottomBar(brand, total, cashback),
                  ],
                ),
              ),
            ),
          ...
          if (minPrice > 0 && minPrice == maxPrice) ...[
            ...
            Wrap(
              ...
              children: denominations.map((d) {
                ...
                return GestureDetector(
                  onTap: () =>
                      setState(() => _selectedAmount = d.toInt().toString()),
                  child: AnimatedScale(
            ...
            if (_selectedAmount != null)
              _cashbackRow(
                ((double.tryParse(_selectedAmount!) ?? 0) *
                        discountPercent /
                        100)
                    .round(),
              ),
          ] else ...[
            _buildVariableAmountInput(minPrice, maxPrice, discountPercent),
          ],
        ],
      ),
    );
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(8),
          border: Border.all(
            color: active ? const Color(0xFF9747FF) : const Color(0xFFDAD5FF),
            width: 2,
          ),
          boxShadow: active
              ? [
  ...
  /// Variable amount input matching the React PaymentDetailsSheet:
  /// hint text "Enter amount between ₹min - ₹max", a text input with
  /// purple focus border, and a live "+₹n cashback" preview below.
  Widget _buildVariableAmountInput(
    double min,
    double max,
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              Text(
                'Enter amount between ₹${min.toInt()} - ₹${max.toInt()}',
                style: GoogleFonts.poppins(
                  fontSize: 11,
                  fontWeight: FontWeight.w500,
        ...
        TextFormField(
          ...
          onChanged: (value) {
            ...
            setState(() => _selectedAmount = raw.isEmpty ? null : raw);
          },
          onFieldSubmitted: (_) {
            if (_selectedAmount != null && _selectedAmount!.isNotEmpty) {
              final val = double.tryParse(_selectedAmount!) ?? 0;
              if (val < min || val > max) {
          ...
          decoration: InputDecoration(
            ...
            enabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
              borderSide: const BorderSide(color: Color(0xFFDAD5FF), width: 2),
            ),
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              if (hasAbout) _buildInfoTabButton('i', 'About', 'about', 71),
              if (hasHowToUse)
                _buildInfoTabButton('*', 'How to Use', 'howtouse', 102),
              if (hasTerms) _buildInfoTabButton('#', 'Terms', 'terms', 73),
            ],
          ),
        ),
        // Expandable content
  ...
  void _showAddToCartSuccess(BuildContext context) {
    showDialog<void>(
      ...
      barrierColor: Colors
          .transparent, // AddToCartSuccessModal paints its own dim backdrop
              ...
              child: GestureDetector(
                onTap: !processing
                    ? () {
                        ...
                        widget.onAddToCart(
                          brand,
                          double.parse(_selectedAmount ?? '0'),
                          _quantity,
                        );
                      }
                    : null,
