...
class _PaymentScreenState extends ConsumerState<PaymentScreen> {
  ...
  final _couponIdController =
      TextEditingController(); // For employee/corporate ID
  String? _appliedCouponCode;
  double _couponDiscount = 0;
  String? _couponError;
  String? _couponType; // 'normal', 'employee', 'corporate'

  /// Detect coupon type from prefix (matches React logic).
  /// Employee: EMP/EMPID prefix → requires employeeId
  /// Corporate: CORP/CORPID prefix → requires corporateId
  String? _detectCouponType(String code) {
    final upper = code.toUpperCase();
    if (upper.startsWith('EMP') || upper.startsWith('EMPID')) return 'employee';
    if (upper.startsWith('CORP') || upper.startsWith('CORPID'))
      return 'corporate';
    return 'normal';
  }
  ...
  Widget build(BuildContext context) {
    ...
    final cashbackRedeemPercent =
        walletAsync.value?.cashbackRedeemPercent ??
        AppConfig.cashbackRedeemPercent;
    final maxRedeemAmount =
        walletAsync.value?.maxRedeemAmount ?? AppConfig.maxRedeemAmount;
    // Compute wallet cap: min(cashbackValue * cashbackRedeemPercent / 100, maxRedeemAmount)
    // cashbackValue = sum of per-item cashback amounts (itemTotal * discount% / 100)
    final cashbackValue = cart.items.fold<double>(0, (sum, item) {
      final discount = double.tryParse(item.discount?.toString() ?? '0') ?? 0;
      final lineTotalPaise = (item.lineTotal * 100).round();
      final pctBasis = (discount * 100).round();
      final product = lineTotalPaise * pctBasis;
      final cashbackScale4 =
          product ~/ 100 + ((product % 100) * 2 >= 100 ? 1 : 0);
      return sum + cashbackScale4 / 10000;
    });
    final walletCapPaise = (cashbackValue * cashbackRedeemPercent / 100).clamp(
      0,
      maxRedeemAmount,
    );
    final maxWalletUsage = walletCapPaise.toDouble();
    final walletDeduction = _useWallet
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '₹${item.unitValue.toStringAsFixed(2)} × ${item.quantity}',
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Available: ₹${walletBalance.toStringAsFixed(2)} • Max: ₹${maxWalletUsage.toStringAsFixed(2)} (50%)',
  ...
  Widget _buildCouponSection(PaymentState paymentState) {
    ...
    final showIdField = _couponType == 'employee' || _couponType == 'corporate';
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      '$_appliedCouponCode applied (-\u20B9${_couponDiscount.toStringAsFixed(2)})',
                      style: TextStyle(
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    onChanged: (value) {
                      final type = _detectCouponType(value.trim());
                      if (type != _couponType) {
                        setState(() {
                          _couponType = type;
                          _couponIdController.clear();
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _isProcessing
                    ? null
                    : () => _handlePayNow(finalPayable, user, items),
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF523DA9),
                  foregroundColor: Colors.white,
                  shape: RoundedRectangleBorder(
    ...
    try {
      ...
      if (!ref.read(paymentProvider).orderCreated) {
        ...
        final created = await paymentNotifier.createOrder(
          ...
          walletAmount: _useWallet
              ? (ref.read(walletBalanceProvider).value?.totalBalance ?? 0) * 0.5
              : 0,
        );
      ...
      final response = await paymentNotifier.validateCoupon(
        CouponValidateRequest(
          ...
          fee: 0,
          employeeId: _couponType == 'employee'
              ? _couponIdController.text.trim()
              : null,
          corporateId: _couponType == 'corporate'
              ? _couponIdController.text.trim()
              : null,
        ),
      );
  ...
  Future<void> _handlePayNow(
    ...
    try {
      ...
      if (!ref.read(paymentProvider).orderCreated) {
        ...
        final walletAmount = _useWallet
            ? (ref.read(walletBalanceProvider).value?.totalBalance ?? 0)
                      .toDouble() *
                  0.5
            : 0.0;
      ...
      // Step 3: Backend-mediated payment initiation (backend computes net payable).
      ...
      // NOTE: Do NOT clear cart here. Cart should only be cleared after
      // successful payment confirmation in PaymentResultScreen (matches React behavior).
      final paymentUrl =
          paymentResponse['payment_url'] ?? paymentResponse['paymentUrl'];
      if (paymentUrl != null && paymentUrl.toString().isNotEmpty && mounted) {
        context.push(
          '/payment-webview',
          extra: {
